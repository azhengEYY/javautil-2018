...
-   rule_name: NO_SALES_FOR_CUSTOMER
    ...
    java_format: >
            etl_customer_id %s ship_to_cust_id %s name: %s has no sales in load %s
-   rule_name: NO_SALES_FOR_CUSTOMER2
    table_name: ETL_CUSTOMER
    msg:      No sales for customer
    sql_text: >
            select ec.etl_customer_id, ship_to_cust_id, ec.cust_nm, etl_file_id
            from etl_customer ec
            where ec.etl_file_id = :ETL_FILE_ID ship_to_cust_id in (
               select ship_to_cust_id from etl_customer where etl_file_id = :ETL_FILE_ID 
               minus  select distinct ship_to_cust_id from etl_sale where etl_file_id = :etl_file_id
